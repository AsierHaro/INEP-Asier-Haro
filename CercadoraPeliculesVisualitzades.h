...
{
public:
    vector<DTOPeliV> cercaVisualitzacions(string sobrenom) {
        ConnexioBD bd;
        std::string sql =
        ...
        vector<DTOPeliV> resultats;
        while (res->next()) {
            DTOPeliV p = DTOPeliV(res->getString("data_formateada"), res->getString("titol_pelicula"), res->getString("descripcio"), res->getString("qualificacio"), res->getInt("num_visualitzacions"));
