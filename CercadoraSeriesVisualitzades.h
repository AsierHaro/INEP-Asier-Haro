...
{
public:
    vector<DTOSerieV> cercaVisualitzacions(string sobrenom) {
        ConnexioBD bd;
        std::string sql =
        ...
        vector<DTOSerieV> resultats;
        while (res->next()) {
            DTOSerieV p = DTOSerieV(res->getString("data_formateada"), res->getString("titol_serie"), res->getString("qualificacio"),res->getInt("num_temporada"), res->getInt("num_capitol"), res->getInt("num_visualitzacions"));
